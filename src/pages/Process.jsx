...
export default function Process(){
 ...
 <InnerHero eyebrow="Our Process" title={<>A considered process.<br/>A refined result.</>} text="From first conversation to final styling, each stage is treated as part of one design language." image={img}/>
 <section className="section process-page" data-reveal><div className="shell process-track process-track--editorial" ref={trackRef}>{steps.map((s,i)=><article key={s[0]}><span>0{i+1}</span><div><h2>{s[0]}</h2><p>{s[1]}</p></div></article>)}</div></section>
